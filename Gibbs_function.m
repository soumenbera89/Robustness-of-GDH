% Energy function along the gradient of NH4......
...
NH4_high=10.0;
NH4_low=1.0;
NH4=linspace(NH4_low,NH4_high,100);
% serve alpha ketoglutarate as random ....
...
%%calculation for energy function.........................
% state vector are s_last(i,1), s_last(i,2), s_last(i,3), s_last(i,4)
% s_last(i,5), s_last(i,6), s_last(i,7), s_last(i,8)
for i=1:length(NH4)
    term = 0;
    sigma_k(i) = s_last(i,1)+s_last(i,2)+s_last(i,3)+s_last(i,4)+s_last(i,5)+s_last(i,6)+s_last(i,7)+s_last(i,8);
    for j=1:8
        n_k(i,j) = s_last(i,j)./sigma_k(i);
        term(j) = s_last(i,j)*(1+log(n_k(i,j)));
    end
    f_n(i)=sum(term);
end
plot(NH4,f_n);
